...
# Cola global para manejar los videos/listas de reproducción
download_queue = queue.Queue()

# Función para descargar videos individuales
def download_video(url, download_path):
    ydl_opts = {
        'format': 'bestvideo[height<=1080]+bestaudio/best[height<=1080]',  # Limitar calidad
        'outtmpl': os.path.join(download_path, '%(title)s.%(ext)s'),  # Guardar el video con su título
...
def download_playlist(url):
    ...
    ydl_opts = {
        'format': 'bestvideo[height<=1080]+bestaudio/best[height<=1080]',  # Limitar calidad
        'outtmpl': os.path.join(download_path, '%(playlist)s/%(title)s.%(ext)s'),  # Guardar en carpetas según la lista
    ...
    while True:
        if not download_queue.empty():
            ...
            if 'playlist' in url:
                ...
                download_path = os.path.join(os.path.expanduser('~'), 'YouTube', 'Videos Individuales')
                os.makedirs(download_path, exist_ok=True)
                print(f"Usando directorio para videos individuales: {download_path}")
...
def main():
    ...
    os.makedirs(base_download_path, exist_ok=True)  # Crear carpeta si no existe
    ...
    while True:
        url = input("Ingresa la URL del video o lista de reproducción de YouTube (o 'salir' para terminar): ")
        if url.lower() == 'salir':
            print("Saliendo del programa...")
            break
        download_queue.put(url)  # Añadir el enlace a la cola
        print(f"Se añadió el enlace {url} a la cola. Total en cola: {download_queue.qsize()}")
